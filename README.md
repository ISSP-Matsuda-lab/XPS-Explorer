...
HからUまでの中性原子のコア準位とオージェ遷移を収録し、Al Kα、Mg Kα、任意の励起光エネルギーに対応しています。中性原子の結合エネルギーは `Core_levels_MM.xlsx` の参照データに基づいています。
...
> [!NOTE]
> `index.html` を直接開くこともできますが、ブラウザーのセキュリティ制限によってサンプルデータを読み込めない場合があります。通常はローカルHTTPサーバーを使用してください。
...
起動時にサンプルの `data/Gr_No1_Ar1.xy` が読み込まれます。`Measurement data` で表示したいRegionにチェックを入れると、その測定スペクトルが `Energy map` に重なります。手元のデータは `Add XY files` から1つ以上追加できます。
...
読み込んだ測定データ、元素の選択、解析結果、プロット設定はブラウザーのIndexedDBに自動保存され、同じブラウザーで次回起動時に復元されます。右上の `Reset all` は保存済みのワークスペースを削除して初期状態へ戻します。
