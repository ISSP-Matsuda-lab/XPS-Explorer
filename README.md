...
HからUまでの中性原子のコア準位とオージェ遷移を収録し、Al Kα、Mg Kα、任意の励起光エネルギーに対応しています。中性原子の結合エネルギーは `Core_levels_MM.xlsx` の参照データに基づいています。オージェ電子の運動エネルギーは NIST X-ray Photoelectron Spectroscopy Database (SRD 20) の推奨元素参照データを優先し、未収録の代表線は LBNL X-Ray Data Booklet (XDB) の Principal Auger Electron Energies を補完参照しています。
